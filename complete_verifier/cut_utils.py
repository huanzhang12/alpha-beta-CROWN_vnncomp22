""" Utilities for cuts. Mainly for legacy cuts and cplex."""
import time
import os
import struct
import psutil
...
def fetch_cut_from_cplex(net):
    start_time = time.time()
    ######## parse cplex cuts files ###########
    pidx = net.c.view(-1).tolist().index(-1)
    # might need to be careful with the obj variable name here, should match to the saving name
    # in build_the_model_mip
    try:
        read_from = 'cplex_processes' if getattr(net, 'cplex_processes', None) is not None else 'processes'
        if read_from == 'processes':
            # local mps
            test_indx = 0
        else:
            # incomplete verifier's mps
            test_indx = net.test
        MODEL_NAME = getattr(net, read_from, None)[test_indx]['_fname_stamped']
    except Exception as e:
        print('Fetch cut process: mps is not yet saved')
        return
    indx_bin = f"{MODEL_NAME}.indx"
    cut_bin = f"{MODEL_NAME}.cuts"

    print("Fetch latest cut from cplex, for label {}".format(pidx))
...
def read_cut_pt(cut_file, no_float64_last_iter=False):
...
def generate_cplex_cuts(net):
    ######## parse cplex cuts files ###########
    # MODEL_NAME = "mip_lay-16_1"
    # MODEL_NAME = "mip_lay-16_0"
    pidx = net.c.view(-1).tolist().index(-1)
    # might need to be careful with the obj variable name here, should match to the saving name
    # in build_the_model_mip
    try:
        ...
        if read_from == 'processes':
            ...
            test_indx = net.test
        MODEL_NAME = getattr(net, read_from, None)[test_indx]['_fname_stamped']
        # MODEL_NAME = f"cplexmip_lay{net.net.final_node().name.replace('/', '-')}_{pidx}"
        indx_bin = f"{MODEL_NAME}.indx"
        cut_bin = f"{MODEL_NAME}.cuts"
        cuts, cut_timestamp = read_cut_efficient(net, cut_bin, indx_bin)
    except Exception as e:
        ...
        for pidx in net.processes:
            ...
            while True:
                if psutil.pid_exists(net.processes[pidx]['pid']):
                    ...
                    break
                time.sleep(0.2)
            # while processes[pidx].poll() is None:
            #     print('kill process for pidx={}'.format(pidx))
            #     processes[pidx].kill()
            #     time.sleep(0.2)
            # Close log file.
