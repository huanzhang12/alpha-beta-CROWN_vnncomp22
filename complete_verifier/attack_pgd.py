...
def test_conditions(input, output, C_mat, rhs_mat, cond_mat, same_number_const, data_max, data_min):
    ...
    if same_number_const:
        ...
        # cond = cond.view(cond.shape[0], cond.shape[1], -1)
    else:
        output = output.repeat_interleave(torch.tensor(cond_mat[0]).to(output.device), dim=2)
        # [num_example, num_restarts, num_spec, num_output]
...
def gen_adv_example(model_ori, x, best_deltas, data_max, data_min, C_mat, rhs_mat, cond_mat):
    ...
    print("pgd prediction:", attack_output[:2,:2])
    ...
    print("pgd attack margin", attack_margin[:2, :, :10])
...
def pgd_attack_with_general_specs(model, X, data_min, data_max, C_mat, rhs_mat, cond_mat, same_number_const, alpha, 
    ...
    best_delta = torch.zeros(input_shape, device=X.device)
...
def boundary_attack(model, x, data_min, data_max):
    ...
    if len(perturbation_index) > 5:
        print("Error: number of perturbed pixels is larger than 5, will not initialize the attack.")
        return None
...
def attack_with_general_specs(model, x, data_min, data_max, list_target_label_arrays, initialization="uniform", GAMA_loss=False):
    ...
    if test_conditions(x, output, C_mat, rhs_mat, cond_mat, same_number_const, data_max.unsqueeze(1), data_min.unsqueeze(1)).all():
        print("clean prediction incorrect")
        return True, x.detach(), output.detach()
    ...
    if test_conditions(attack_image.unsqueeze(1), attack_output.unsqueeze(1), C_mat, rhs_mat, cond_mat, same_number_const, data_max, data_min).all():
        print("pgd attack succeed!")
        return True, attack_image.detach(), attack_margin.detach()
    else:
        print("pgd attack failed")
        return False, attack_image.detach(), attack_margin.detach()
