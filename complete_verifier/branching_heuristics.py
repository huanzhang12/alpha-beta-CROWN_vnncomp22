        ...
        for k in range(topk):
            ...
            if use_beta:
                k_ret_lbs = net.update_bounds_parallel(lbs, ups, k_decision[-1], sps if set_slope else [], betas=bs,
                                                       layer_set_bound=True, shortcut=True, history=history, cs=cs)
            else:
                k_ret_lbs = net.update_bounds_parallel(lbs, ups, k_decision[-1], sps if set_slope else [], beta=False,
                                                       layer_set_bound=True, shortcut=True, cs=cs)
...
def choose_node_parallel_kFSB(lower_bounds, upper_bounds, orig_mask, net, pre_relu_indices, lAs,
    ...
    if method == 'kfsb-intercept-only':
        ...
        raise ValueError("Branching method invalid.")
    ...
    for k in range(topk):
        ...
        if use_beta:
            k_ret_lbs = net.update_bounds_parallel(lbs, ups, k_decision[-1], sps if set_slope else [], betas=bs,
                                                   layer_set_bound=True, shortcut=True, history=history, cs=css)
        else:
            k_ret_lbs = net.update_bounds_parallel(lbs, ups, k_decision[-1], sps if set_slope else [], beta=False,
                                                   layer_set_bound=True, shortcut=True, cs=css)
...
def input_split_branching(net, dom_lb, dm_l_all, dm_u_all, lA, thresholds, branching_method, selected_dims, shape, slopes, split_depth=1):
    ...
    if branching_method == 'naive':
        ...
        for dim in range(dm_l_all.shape[-1]):
            ...
            k_ret_lbs = net.get_lower_bound_naive(dm_l=new_dm_l, dm_u=new_dm_u, slopes=slopes, shortcut=True)
            # we only consider the best lower bound across two splits by using min(0)
            k_ret[dim] = k_ret_lbs.reshape(2, -1).sum(-1, keepdim=True).min(0).values

        i_idx = k_ret.max(0).indices  # compare across input dims
        i_idx = i_idx.unsqueeze(1)
    elif branching_method == 'sb-fast':
...
def get_split_depth(dm_l_all):
    split_depth = 1
    if len(dm_l_all) < arguments.Config["solver"]["beta-crown"]["min_batch_size_ratio"] * arguments.Config["solver"]["beta-crown"]["batch_size"]:
        min_batch_size = arguments.Config["solver"]["beta-crown"]["min_batch_size_ratio"] * arguments.Config["solver"]["beta-crown"]["batch_size"]
