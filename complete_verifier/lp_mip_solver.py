...
def mip_solver_attack(new_splits):
    ...
    input_shape = list(input_shape)
    unstable_neuron_cnt = 0
    init_unstable_neuron_cnt = 0
    refined_cnt = 0
    lb_improved, ub_improved = 0, 0
...
def mip_solver_lb_ub(candidate, init=None, save_adv=None):
    ...
    if arguments.Config["solver"]["mip"]["early_stop"]:
        if arguments.Config["solver"]["intermediate_refinement"]["solver_pkg"] == 'gurobi':
            model.setParam('BestBdStop', 1e-5)  # Terminiate as long as we find a positive lower bound.
            model.setParam('BestObjStop', -1e-5)  # Terminiate as long as we find a adversarial example.
        elif arguments.Config["solver"]["intermediate_refinement"]["solver_pkg"] == 'scip':
    ...
    if arguments.Config["solver"]["intermediate_refinement"]["solver_pkg"] == 'scip':
...
def build_solver_model(m, timeout, mip_multi_proc=None,
    ...
    m.net.solver_pkg = arguments.Config["solver"]["intermediate_refinement"]["solver_pkg"]
    ...
    out_vars = BoundedModule.build_solver_module(m.net, C=m.c, final_node_name=m.net.final_name, model_type=model_type, solver_pkg=m.net.solver_pkg)
    m.net.model.update()
...
def update_the_model_cut(m, cut, pre_lbs=None, pre_ubs=None, split=None, verbose=False):
    ...
    # for c in m.model_cut.getConstrs():
        # print('The dual value of %s : %g %g'%(c.constrName, c.pi, c.slack))
    # import pdb; pdb.set_trace()

    # assert m.model_cut.status == 2, f"LP wasn't optimally solved status:{m.model_cut.status}"
...
@torch.no_grad()
def construct_mip_with_model(unwrapped_model, x, y, target, input_shape, c, intermediate_bounds,
        labels_to_verify=None, save_mps=False, process_dict=None):
    ...
    model = beta_CROWN_solver.LiRPAConvNet(unwrapped_model, y, target, in_size=input_shape, c=c, device='cpu')
    # Now setup all input bounds and intermediate layer bounds.
    model.net._set_input(x, new_interval=intermediate_bounds)
    build_the_model_mip(model, labels_to_verify=labels_to_verify, save_mps=save_mps, process_dict=process_dict)


# updated function using general computation graph to build lp model
# @torch.no_grad()
def build_the_model_mip(m, labels_to_verify=None, save_mps=False, process_dict=None, save_adv=False):
    """
    ...
    build_solver_model(m, timeout, mip_multi_proc=mip_multi_proc, 
                    mip_threads=mip_threads, model_type="mip")

    out_vars = m.net[m.net.final_name].solver_vars
    lb = m.net.final_node().lower[0].tolist()
    ub = [float('inf') for _ in lb]

    # out_vars and lb do not have var for predicted label, add a None and 0. 
    # to make order correct for label printout
    out_vars.insert(m.pred, None)
    lb.insert(m.pred, 0.)
    ub.insert(m.pred, 0.)
    ...
    if save_mps:
        ...
        for cidx, candidate in enumerate(candidates):
            if m.c.shape[1] == 1:
                # simplified to only one label
                pidx = m.c.view(-1).tolist().index(-1)
                model_filename = os.path.join(CPLEX_FOLDER, f"cplexmip_lay{m.net.final_node().name.replace('/', '-')}_starttime={arguments.Globals['starting_timestamp']}_idx={arguments.Globals['example_idx']}_label={pidx}")
            else:
                # not simplifed
                model_filename = os.path.join(CPLEX_FOLDER, f"cplexmip_lay{m.net.final_node().name.replace('/', '-')}_starttime={arguments.Globals['starting_timestamp']}_idx={arguments.Globals['example_idx']}_label={candidate_neuron_ids[cidx]}")
            model_filename_stamped = model_filename + '_' + gen_timestamp()
            model_filename_stamped_dict[candidate_neuron_ids[cidx]] = model_filename_stamped
        ...
        # for cidx, candidate in enumerate(candidates):
        #     # v = m.net.model.getVarByName(candidates[label_id])
        #     # model_filename = 'cplexmip_' + candidates[label_id].replace('/', '-')
        #     v = m.net.model.getVarByName(candidate)
        #     # model_filename = 'cplexmip_' + candidate.replace('/', '-')
        #     model_filename = f"cplexmip_lay{m.net.final_node().name.replace('/', '-')}_{candidate_neuron_ids[cidx]}"
        #     model_filename_stamped = model_filename + '-' + gen_timestamp()
        #     m.net.model.setObjective(v, grb.GRB.MINIMIZE)
        #     print(f"save mip model to {model_filename_stamped}.mps")
        #     m.net.model.write(f'{model_filename_stamped}.mps')
        #     model_filename_stamped_dict[candidate_neuron_ids[cidx]] = model_filename_stamped
        ...
        for cidx, candidate in enumerate(candidates):
            ...
            model_filename_stamped = model_filename_stamped_dict[candidate_neuron_ids[cidx]]
            try:
                ...
                processes[candidate_neuron_ids[cidx]] = {'pid': proc.pid, '_logfile': logfile, '_fname_stamped': model_filename_stamped}
    ...
    if adv_warmup:
        ...
        max_values.insert(m.pred, 0.)
        min_values.insert(m.pred, 0.)
        layer_size = m.net.final_node().forward_value.size(-1)
        adv = adv.cpu().tolist()
        adv.insert(m.pred, None)
        adv.insert(layer_size + m.pred, None)
        adv_list = []
        for neuron_idx in candidate_neuron_ids:
            ...
            # print(neuron_idx, min_values[neuron_idx])
...
def FSB_score(net, lower_bounds, upper_bounds, orig_mask, pre_relu_indices, lAs, branching_candidates=5, 
            branching_reduceop='min', slopes=None, bound_reshaped=False):
    """
    ...
    intercept_tb = []
    relu_idx = -1
    number_bounds = lower_bounds[-1].shape[-1]
    if bound_reshaped:
        # one bound is added after bound calculation
        number_bounds -= 1
            ...
            if need_refine and (relu_idx >= 1 and relu_idx < len(m.net.relus)) and (time.time() - mip_refine_time_start < mip_refine_timeout):
                ...
                if relu_idx == 1:
                    if adv_warmup:
                        ...
                        adv_list = []
                        layer_size = layer.weight.size(0)
                        for neuron_idx in candidate_neuron_ids:
                    ...
                    # m.net.set_bound_opts({'optimize_bound_args': {'ob_beta': True}, 
                    #             'enable_opt_interm_bounds': arguments.Config['solver']['beta-crown']['enable_opt_interm_bounds']})
                    # # m.net.set_bound_opts({'optimize_bound_args': {'ob_beta': False, 'ob_verbose': True, 'ob_lr_beta': 0.}})
                    ...
                    with multiprocessing.Pool(mip_multi_proc) as pool:
                        ...
                        if last_relu_layer_refined and (time.time() - mip_refine_time_start < mip_refine_timeout):
                            ...
                            m.net.set_bound_opts({'optimize_bound_args': {'ob_beta': True, "verbose":True}, 
                                'enable_opt_interm_bounds': arguments.Config['solver']['beta-crown']['enable_opt_interm_bounds']})
