...
def get_sparse_C(
    ...
    max_crown_size = self.bound_opts.get('max_crown_size', 1e9)
...
def get_alpha_crown_start_nodes(
    ...
    for nj in self.backward_from[node.name]:  # Pre-activation layers.
        ...
        if (sparse_intermediate_bounds and isinstance(node, BoundRelu) 
            ...
            # Create sparse optimzation variables for intermediate neurons.
            if ((isinstance(nj, BoundLinear) or isinstance(nj, BoundMatMul)) 
                ...
                if nj.name in node.patch_size:
                    ...
                    unstable_idx, _ = self.get_unstable_locations(nj.lower, nj.upper)
                    use_sparse_conv = True  # alpha is not shared among channels, and is spasre in spec dimension.
        ...
        if share_slopes:
            ...
            assert use_sparse_conv is False  # Double check our assumption holds. If this fails, then we created wrong shapes for alpha.
        else:
            # Output is linear layer, or patch converted to matrix.
            assert use_sparse_conv is not False  # Double check our assumption holds. If this fails, then we created wrong shapes for alpha.
