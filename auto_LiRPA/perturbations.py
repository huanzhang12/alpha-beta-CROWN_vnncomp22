...
from .patches import Patches, patches_to_matrix, inplace_unfold
    ...
    def concretize_patches(self, x, A, sign, extra_constr):
        if self.norm == np.inf:
            ...
            if not A.identity == 1:
                ...
                unfold_input = inplace_unfold(center, kernel_size=A.patches.shape[-2:], padding=A.padding, stride=A.stride, inserted_zeros=A.inserted_zeros, output_padding=A.output_padding)
                ...
                unfold_diff = inplace_unfold(diff, kernel_size=A.patches.shape[-2:], padding=A.padding, stride=A.stride, inserted_zeros=A.inserted_zeros, output_padding=A.output_padding)
                if A.unstable_idx is not None:
                    # We need to add a out_c dimension and select from it.
