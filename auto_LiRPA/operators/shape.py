    ...
    def bound_backward(self, last_lA, last_uA, x, shape):
        def _bound_oneside(A):
            if A is None:
                return None
            if type(A) == Patches:
                if isinstance(self.inputs[0], BoundLinear):
                ...
                if A.unstable_idx is None:
                    ...
                    next_A = patches_to_matrix(patches, [self.input_shape[0]*self.input_shape[1], patches.shape[-3], int(math.sqrt(self.input_shape[-1]//patches.shape[-3])), int(math.sqrt(self.input_shape[-1]//patches.shape[-3]))], A.stride, A.padding)
                else:
...
class BoundGather(Bound):
    ...
    def bound_backward(self, last_lA, last_uA, x, indices):
        ...
        def _expand_A_with_zeros(A, axis, idx, max_axis_size):
            ...
            tensors.append(A)
            if self.input_shape[axis] - idx - 1 > 0:
