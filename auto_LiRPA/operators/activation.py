...
from ..utils import unravel_index, logger
...
class BoundRelu(BoundOptimizableActivation):
    ...
    def init_opt_parameters(self, start_nodes):
        ...
        # We first deal with the sparse-feature alpha, which is sparse in the C*H*W dimesnion of this layer.
        ...
        if (hasattr(self.inputs[0], 'lower') and hasattr(self.inputs[0], 'upper') 
            ...
            # Since each element in a batch can have different unstable neurons, for simplicity we find a super-set using any(dim=0).
            ...
            if self.alpha_indices[0].size(0) <= minimum_sparsity * total_neuron_size:
                ...
                if len(self.alpha_indices) == 1:
                    ...
                    alpha_init = self.lower_d[:, :, self.alpha_indices[0], self.alpha_indices[1], self.alpha_indices[2]]
                else:
                    raise ValueError
            ...
            alpha_shape = self.shape  # Full alpha.
            alpha_init = self.lower_d
        # Now we start to create alphas for all start nodes. When sparse-spec feature is enabled, alpha is created for only unstable neurons in start node.
        for ns, output_shape, unstable_idx in start_nodes:
            if isinstance(output_shape, (list, tuple)):
                ...
                size_s = output_shape
            # unstable_idx may be a tensor (dense layer or conv layer with shared alpha), or tuple of 3-d tensors (conv layer with non-sharing alpha).
            ...
            if sparsity <= minimum_sparsity * size_s and self.use_sparse_spec_alpha:
                ...
                if isinstance(output_shape, int) or len(output_shape) == 1:
                    # Fully connected layers, or conv layer in patches mode.
                ...
                print(f'layer {self.name} start_node {ns} using full alpha with unstable size {unstable_idx.size() if unstable_idx is not None else None} total_size {size_s} output_shape {output_shape}')
    ...
    def bound_dynamic_forward(self, x, max_dim=None, offset=0):
        ...
        # logger.debug(f'Actual new dimensions for this batch: {num_new_dim_actual}')
    ...
    def _backward_relaxation(self, last_lA, last_uA, x, start_node, unstable_idx):
        ...
        if hasattr(x, 'interval') and Interval.use_relative_bounds(x.interval):
            # Relative interval bound (center, diff) (not used).
            ...
            if unstable_idx is not None and self.alpha[start_node.name].size(1) != 1:
                ...
                if isinstance(unstable_idx, tuple):
                    ...
                    if isinstance(last_lA, Tensor) or isinstance(last_uA, Tensor):
                        ...
                        # If sparse-spec alpha is used, alpha will be selected later in maybe_unfold().
                ...
                selected_alpha = self.alpha[start_node.name]
            # print(f'relu layer {self.name} start node {start_node.name} selected_alpha shape {selected_alpha[0].size()}')
    ...
    def bound_backward(self, last_lA, last_uA, x=None, start_node=None, beta_for_intermediate_layers=False, unstable_idx=None):
        ...
        if self.cut_used:
            ...
            last_lA, last_uA = self.cut_module.relu_cut(start_node, self.name, last_lA, last_uA, current_layer_shape, unstable_idx,
        ...
        if self.cut_used:
            ...
            lA, uA, lbias, ubias = self.cut_module.arelu_cut(start_node, self.name, last_lA, last_uA, lower_d, upper_d,
                                                             lower_b, upper_b, lb_lower_d, ub_lower_d, I, x, self.patch_size, current_layer_shape, unstable_idx,
                                                             batch_mask=self.alpha_beta_update_mask)
        else:
        ...
        if self.options.get('optimize_bound_args', {}).get('ob_beta', False) and self.sparse_beta is not None:
            if self.options.get('optimize_bound_args', {}).get('ob_single_node_split', False):
                ...
                # assert type(A) is not Patches or unstable_idx == A.unstable_idx
            ...
            # elif False and not self.options.get('optimize_bound_args', {}).get('ob_single_node_split', True):
    ...
    def interval_propagate(self, *v):
        if Interval.use_relative_bounds(*v):
            ...
            mask_nominal = (nominal > 0).to(nominal.dtype)
            mask_l = (v[0].lower > 0).to(nominal.dtype)
            mask_u = (v[0].upper > 0).to(nominal.dtype)
    ...
    def build_solver(self, *v, model, C=None, model_type="mip", solver_pkg="gurobi"):
        ...
        # pre_layer_shape (8,16,16)
        this_layer_shape = pre_layer_shape = gvars_array.shape
        ...
        for neuron_idx, pre_var in enumerate(gvars_array.reshape(-1)):
            ...
            if pre_lb >= 0:
                ...
                # assert self.I.view(-1)[neuron_idx]
                lb = 0
                ub = pre_ub
                ...
                # name=f'ReLU{self.name}_[{chan_idx},{row_idx},{col_idx}]')
    ...
    def interval_propagate(self, *v):
        ...
        if Interval.use_relative_bounds(*v):
            ...
            if exp % 2 == 0:
                ...
                mask = 1 - ((h_L < 0) * (h_U > 0)).to(h_L.dtype)
                lower = lower * mask
            return Interval.make_interval(lower, upper, nominal=self.forward_value, use_relative=True)
...
class BoundATenHeaviside(BoundOptimizableActivation):
    ...
    def init_opt_parameters(self, start_nodes):
        ...
        for ns, size_s, _ in start_nodes:
            ...
            # self.alpha[ns][0].data.copy_(self.upper_d)
            # self.alpha[ns][0].data.copy_(self.lower_d)
            # del self.upper_d, self.lower_d
    ...
    def bound_backward(self, last_lA, last_uA, *x, start_node=None, start_shape=None):
        ...
        if self.opt_stage not in ['opt', 'reuse']:
            ...
            # adaptive crown (TODO: the code is ugly and very inefficient).
            # unstable_mask = (lb_r < 0) & (ub_r > 0)
            # upper_d[(-lb_r > ub_r) & unstable_mask] = 1. / lb_r[(-lb_r > ub_r) & unstable_mask]
            # lower_d[(ub_r > -lb_r) & unstable_mask] = 1. / ub_r[(ub_r > -lb_r) & unstable_mask]
        else:
