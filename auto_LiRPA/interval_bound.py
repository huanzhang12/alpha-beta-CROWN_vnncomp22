...
def _IBP_loss_fusion(self, node, C):
    ...
    if C is None and isinstance(node, BoundSub):
        ...
        if isinstance(node_gather, BoundGatherElements):
...
def check_IBP_first_linear(self, node):
    ...
    Disable this optimization when we need the A matrix of the first nonlinear layer, 
    forcibly use CROWN to record A matrix.
    """
    ...
    if (node.name not in self.needed_A_dict.keys() 
            and (type(node) == BoundLinear 
                or type(node) == BoundConv and node.name not in intermediate_beta_enabled_layers)):
